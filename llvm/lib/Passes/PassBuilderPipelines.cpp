...
void PassBuilder::addVectorPasses(OptimizationLevel Level,
  ...
  if (isFullLTOPostLink(LTOPhase)) {
    ...
    // We also turn on memory-intrinsic-only canonicalization here. This allows
    // promotion by converting homogeneous structs to vectors and small
    // memcpy-only partitions to legal integers. We only turn this on after
    // memcpyopt runs because this might hinder memcpyopt's optimizations if
    // done before. Look at the documentation in SROA.cpp to see why.
